...
*****************************************************************************
 *
 * @file yuvconverter.cpp
 *
 * @brief Convert Convert one YUV format to another
*****************************************************************************/
...
int main(int argc, char **argv)
{
    ...
    ptrbufinframe = bufinframe;
    ptrbufoutframe = bufoutframe;
    fread(bufinframe, 1, inframesize, infile);
        
    while (!feof(infile))
    {
        yuvconvert(ptrbufinframe, ptrbufoutframe,width, height);
        fwrite(bufoutframe, 1, outframesize, outfile);
        fread(bufinframe, 1, inframesize, infile);
    }
